    ...
    public static class HtmlHelperExtension
{
    ...
    public class Html5Form
    {
        ...
        private IHtmlString Build(string type)
        {
            ...
            var placeHolder = !string.IsNullOrWhiteSpace(metadata.Watermark)
            ...
            string input = string.Format(Culture,
                "<input type=\"{0}\" id=\"{1}\" name=\"{2}\" class=\"{3}\" {4} {5} {6}/>", type, id, name, cssClass,
                valueAttribute, placeHolder, validators);
        ...
        private string GetValidators(ModelMetadata metadata)
        {
            ...
            foreach (var rule in validatorsDictionary)
            {
                if (rule is ModelClientValidationRequiredRule)
                {
                    ...
                    {
                        rules.AppendFormat(Culture, " data-val-required=\"{0}\"",
                            HttpUtility.HtmlEncode(rule.ErrorMessage ?? string.Empty));
                    }
                ...
                else if (rule is ModelClientValidationRegexRule)
                {
                    ...
                    if (unobtrusiveJavaScriptEnabled)
                    {
                        rules.AppendFormat(Culture, " data-val-pattern=\"{0}\"",
                            HttpUtility.HtmlEncode(rule.ErrorMessage ?? string.Empty));
                    }
                }
                else if (rule is ModelClientValidationRangeRule)
                {
                    ...
                    if (unobtrusiveJavaScriptEnabled)
                    {
                        rules.AppendFormat(Culture, " data-val-range=\"{0}\"",
                            HttpUtility.HtmlEncode(rule.ErrorMessage ?? string.Empty));
                    }
                }
                else
                {
                    ...
                    {
                        rules.AppendFormat(Culture, " {0}=\"{1}\"", dictionaryKey + pair.Key,
